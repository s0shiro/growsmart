...
interface Props {
  status: 'inspection' | 'harvest' | 'harvested'
  useFetchData: () => { data: Crop[] | null; isLoading: boolean; error: any }
  tableTitle: string
  actions: Action[]
}

export default function ReusableCropTable({
  status,
  useFetchData,
  tableTitle,
  actions,
}: Props) {
  const { data: crops, isLoading, error } = useFetchData()
  ...
  const clearFilters = () => {
    setFilters({ cropName: 'all', fieldLocation: 'all' })
    setCurrentPage(1)
  }
