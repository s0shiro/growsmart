...
export default function HarvestedCropTable() {
  ...
  }, [harvestedCrops, searchTerm, filterCrop])
  ...
  const cropTypes = useMemo(
    () => [
      ...new Set(
        Array.isArray(harvestedCrops) ? harvestedCrops.map((crop: any) => crop.crops.name) : [],
      ),
    ],
    [harvestedCrops],
  )
            ...
            {isFetching
              ...
              : paginatedCrops.map((crop: any) => (
                <TableRow key={crop.id}>
                  ...
                  <TableCell>{formatDate(crop.harvest_records[0]?.harvest_date)}</TableCell>
